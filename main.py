...
def transpose(input, filters, kernel_size, strides = (2, 2), stdev = 0.01, reg = 1e-3):
    return tf.layers.conv2d_transpose(input, filters, kernel_size, 
                                    strides = strides, 
                                    padding = 'same', 
                                    kernel_initializer = tf.random_normal_initializer(stddev=stdev), 
                                    kernel_regularizer = tf.contrib.layers.l2_regularizer(reg))

def conv(input, filters, kernel_size, stdev = 0.01, reg = 1e-3):
    return tf.layers.conv2d(input, filters, kernel_size, 
                            padding = 'same', 
                            kernel_initializer = tf.random_normal_initializer(stddev=stdev), 
                            kernel_regularizer = tf.contrib.layers.l2_regularizer(reg))
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    layer3a_in2 = conv(vgg_layer3_out, num_classes, 1)
    # 1x1 convolution of vgg layer 4
    layer4a_in2 = conv(vgg_layer4_out, num_classes, 1)
    # 1x1 convolution of vgg layer 7
    layer7a_out = conv(vgg_layer7_out, num_classes, 1)
    # upsample
    layer4a_in1 = transpose(layer7a_out, num_classes, 4)
    # skip connection (element-wise addition)
    layer4a_out = tf.add(layer4a_in1, layer4a_in2)
    # upsample
    layer3a_in1 = transpose(layer4a_out, num_classes, 4)
    # skip connection (element-wise addition)
    layer3a_out = tf.add(layer3a_in1, layer3a_in2)
    # upsample
    nn_last_layer = transpose(layer3a_out, num_classes, 16, strides = (8, 8))
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for i in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            ...
            counter += 1
        print()
        print("Average loss: = {:.3f}".format(total_loss / counter))
        print()
